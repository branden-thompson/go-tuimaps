...
- `Set` still never waits on anything; for these shapes it does bounded work. **Measured in BUILD (task 10.26), at the vertex cap of 2,000,000: 23 ms with the borrow check off and 25 ms with it on** - about 11 nanoseconds a vertex, so 1 ms at 100,000 vertices. *PLAN's estimate, "a few milliseconds at the vertex cap by arithmetic", was low by about eight times.* A host that sets shapes of that size should know that `Set` is an owner call on its interface goroutine. *An earlier version had such a shape go undrawn until its replacement was prepared; that was the coordinator's, unruled, and D-92 replaces it.*
