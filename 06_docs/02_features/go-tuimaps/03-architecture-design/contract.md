...
- `Set` still never waits on anything; for these shapes it does bounded work — a few milliseconds at the vertex cap by arithmetic, measured by task 10.26 with the borrow check on and off. *An earlier version had such a shape go undrawn until its replacement was prepared; that was the coordinator's, unruled, and D-92 replaces it.*
