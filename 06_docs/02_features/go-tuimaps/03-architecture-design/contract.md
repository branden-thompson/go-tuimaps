...
- **A shape large enough that it might have to be drawn straight from the host's memory** (FR-11's fallback) has no such copy to fall back on. So for any feature overlay of more than 31,250 vertices at the default shape cap — the number that would fill the cache unsimplified — **`Set` itself makes one linear pass and builds the run index that drawing from memory needs** (D-92). The new shape is drawable on the next frame, the old geometry is released at once, and nothing vanishes. `Set` still never waits on anything; for these shapes it does bounded work — a few milliseconds at the vertex cap by arithmetic, measured by task 10.26. *An earlier version of this bullet had such a shape go undrawn until its replacement was prepared; that was the coordinator's, unruled, and D-92 replaces it.*
