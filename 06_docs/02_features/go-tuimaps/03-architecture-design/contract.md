...
| Class | Calls | Rule |
|---|---|---|
| **Owner** | `Render`, `Set`, `Remove`, the intents, `SetPlaces`, `AddPlace`, `RemovePlace`, every look and tile setting, the playback calls, `OnPending`, `Purge`, `Verify`, `Describe`, `Legend`, `Credits`, `Scale`, `Footer`, `Warnings`, `NextCall` | **One at a time.** They are not safe against each other from two goroutines. Safe beside any pump call |
