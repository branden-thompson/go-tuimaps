...
| Areas with no colour | Hatched, by severity: extreme and severe a cross stroke every 2 and 3 cells, moderate a back stroke every 4, minor and unknown a forward stroke every 5 and 6. A cell is hatched when its column and row together are a multiple of the stride, and only where nothing else has taken the cell, so a marker or a name inside the area always shows | Set in BUILD (task 09.15), from specimen 13a's hatch and S13-1 |
| Markers | A dot is 3 dots by 3; a cross reaches 3 each way; a diamond has radius 3; a ring and a filled circle are drawn by the midpoint rule at the radius given, 3 by default. Kept while within 20 dots of the rectangle, culled beyond it; the label begins 4 dots to the right (P-58, P-60) | Upstream's, read in DISCOVER |
| Blink | 800 ms - upstream's eight ticks on and eight off, at its 50 ms loop (P-59a). Two changes a second, inside the 2.5 a second ceiling (D-56). A clock that does not advance draws the marker on (NFR-21) | Upstream's rate; the ceiling is ruled |
| Stale mark | The word `stale` at the right end of the first row, in its own token (FR-32) | Set in BUILD (task 09.16) |
| Footer | The host's own line at the left of the row above the last, off unless the host gives it (P-57) | Set in BUILD (task 09.16) |
...
| Temperature preset | 17 classes; breaks every 5 °C from −30 to +45; pale break at 0 °C; °F breaks are these converted exactly. **Two sets of colours, chosen by the ground in effect:** they differ in three bands, −10 to +5 °C (D-91) **Order:** relative luminance rises to freezing and falls from it, as FR-16 defines order. In truecolor the same holds for lightness under all three simulated kinds of colour vision; at 256 colours it does not quite — three neighbouring pairs invert by 0.1 to 1.6 lightness units under protanopia or tritanopia, because that palette is coarse; *and, found in BUILD (task 08.8), a fourth on the dark ground only: the two pale classes either side of freezing swap under protanopia by 0.4 (98.6 then 98.2). The preset's test holds the exact count, four on a dark ground and three on a light one, so a change is noticed.* **The pale class the scale is lightest at is not the same in every variant:** class 6, just below freezing, in truecolor on a dark ground; class 7, just above, on a light ground and at 256 colours. Radar's ramps hold under every kind at both depths | D-62, D-91, specimen 21 |
| Alert preset | **Designed in BUILD (task 08.10): five severities, an outline and a tint each, two sets by ground and two by depth - magenta, red-orange, amber, green-blue, grey. Every rule passes on all four; worst pair 12.6 on a dark ground, 16.2 on a light one, 14.5 and 11.7 at 256 colours. Seen by HUM LEAD on a map, specimen 28, and found fine (D-113); a swatch is specimen 27.** *As it stood in PLAN:* not yet designed in full. The two pairs PLAN's specimens drew pass on the dark ground: outlines 21.2 apart, tints 12.6, every colour at least 25.6 from the ground, outline on its own tint at least 4.3:1. **On a light ground both outlines fail 3:1** (1.3 and 2.3), so alerts need a light-ground set as radar and temperature do; and three of the five severities have never been drawn. Designed in task 08.10, shown to HUM LEAD before reference frames are frozen | Measured here (P2-PRD-18); owed in BUILD |
