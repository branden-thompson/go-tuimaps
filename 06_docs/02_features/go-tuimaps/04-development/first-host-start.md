...
| 1a | The host's repository | **After some of the host's own code imports the library**, run the toolchain's tidy step, so the checksum file agrees. Tidy before that and it deletes the requirement again, keeping only the override - found by doing it in that order during the first integration (task 14.19) | **This may touch the network**: if the library asks for a newer version of either text module than the host has, the newer one is fetched. With the versions the host pins today, nothing is |
