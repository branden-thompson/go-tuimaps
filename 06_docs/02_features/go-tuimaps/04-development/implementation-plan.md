...
| Separate modules resolve the library from this tree | The gate script writes a throw-away workspace file, **and inside it a `replace` for the library at exactly `v0.0.0`** — found in BUILD: once the library imports third-party code, a workspace alone sends the toolchain looking for the unpublished version. Tracked module files carry no `replace`. The allow-list test runs with the workspace switched off (D-81) |
