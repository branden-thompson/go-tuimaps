...
| S22-4 | **Found by the PLAN red-team (PL-AX-1):** the ramp above works on a dark ground only. On the painted light ground of specimen 20 its **heaviest** class is 3.1 from the ground and 1.01:1 — a 60 dBZ core would read as a dry hole (, kept as the record of the defect). The checker had no rule about the ground. A second ramp, **darker means heavier**, is chosen when the ground is light (): every pair at least 16.0 apart (12.6 in the 256 palette), every class at least 18.7 from the ground. | The radar preset is two ramps, chosen by the ground's luminance as the style is; the checker gains a ground rule (D-88).  awaits HUM LEAD's eyes. |
