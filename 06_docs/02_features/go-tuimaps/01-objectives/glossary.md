...
| **Work** | The call by which a host lets the library do one piece of slow work — fetch and decode a tile, prepare an overlay, compute a description — on the host's own goroutine. The library never does such work unasked (D-73). |
| **Pump** | The few lines a host writes to keep calling `Work` while work is pending, from one or two goroutines of its own. Its width — how many run at once — and the memory that costs are the host's (D-84). |
| **Owner** | The one goroutine of a host, usually its interface's, that makes every call except `Work` and `Settle`. |
| **Live view** | The current view and size of a map that has not been closed, whether or not it has rendered. What live views are drawing is never evicted from a cache (D-90). |
| **Need** | The bytes live views are drawing: their tiles on hand, the ancestor tiles standing in for ones not yet arrived, their overlays' simplified shapes and run indexes. A cache holds at least its need, even over its cap, and says so. |
| **Run index** | One bounding box for each run of 64 vertices of a very large borrowed shape, built inside `Set`, so the shape can be drawn straight from the host's memory and culled without reading all of it (D-92). |
