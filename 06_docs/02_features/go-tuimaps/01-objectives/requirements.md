...
| FR-9 | Accept georeferenced images, each with a colour-to-intensity table, and re-colour them with the library's ramp. *Whether the table is mandatory is Q4 (D-39).* | D-14, D-36, D-39 |
...
| NFR-17 ✚ | A runnable, continuously tested example for each input shape lives under `examples/`, outside the library's contract — including radar from one keyless source, complete. | N-1, D-15, D-36 |
