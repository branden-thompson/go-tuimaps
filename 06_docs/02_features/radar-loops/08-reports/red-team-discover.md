...
sev: SEV-0
authority: HUM LEAD
status: "ROUND 2 DISPOSITIONED — every finding fixed or ruled (D-39..D-48)"
---
...
|---|---|---|
| Radar erases furniture at 16 colours too (A7) | `rampless` is NoColour **and** Colours16 (`internal/render/field.go:103-105`); specimen 29 re-rendered at `Colours16` | **Confirmed, and wider:** label, marker and credit gone at both sizes, scale gone at 69×12, and ~~every rain cell drawn as `░`~~ *(withdrawn, D-50: a miscount — 16 colours draws the same three shades as no colour)* |
...
| # | Finding | From | Severity | Disposition |
|---|---|---|---|---|
| R2-1 | **The gate still reports green without checking**: combined mode flags run nothing (C F-3); `--fuzz` with `go test -list` errors gives zero legs; the licence loop checks nothing in nested modules (C F-2); NOT RUN's tag lookup fails under squash-merge (C F-1, H-3); the KILL fallback cannot fire, and the flag file races (C F-4, S-8); the run log cannot name the tree it tested, records no override (`GATE_FUZZ_LIMIT`, `GATE_ROOT`) and tests only the green path (C F-5, F-6, F-10, H-5, S-9); D-31(4) has no test (C F-8); "STALLED" misnames a time limit (C F-9); the atlas rebuild instruction fails (C F-7); README omits perl and the workspace (H-2); `compose` sorts its caller's slice | C, H, S | Important | **FIXED — D-40**, test-first, each fix mutation-proven: one mode per run; `GATE_FUZZ_LIMIT` validated; tests or a module graph that cannot be listed fail; a fuzz run with no leg fails; the limiter reports through the watchdog's exit status and always KILLs after TERM (a TERM-ignoring child is killed, and a test checks nothing survives); the last tag found by listing tags; the run log names the tested tree and any override, and a failed run is logged as FAILED; "TIME LIMIT" replaces "STALLED"; mode-specific NOT RUN lines; the atlas takes `-root` and its instructions work; `compose` sorts a copy; README names perl and the workspace. Licences are now checked in `--quick` as well |
| R2-2 | **"The docs lane judges the tree, not the commit" was dropped from R1-10 without a ruling** | C F-5, H-4 | Important | **RULED — D-41**: the lane judges the staged change |
| R2-3 | **Four security rows can be met while their exploit works**: L-1.14 (S-1: copy every host slice first — PNG and table, single image too — and validate the copy); L-7.3 (S-2: the library bounds bytes and time of every host fetch itself); L-9.3 (S-4: no write after Purge, CacheRoot change/off, or Close; replaced roots released); L-9.4 (S-3: age from fetch time, never refreshed by a read). New: a proxy disables the private-address check, and the reserved-range list is incomplete (S-5). Minor: a readable cache root (S-6); `ReadBack` reads before its limit, fields outside the budget, `ImageKey` collisions (S-7); L-5.5 does not require a clean scan (S-9); a patch adds `os.Getenv` to the renderer (S-10) | S-1–S-10 | Important | **FIXED — D-40**: L-1.14, L-7.3, L-9.3, L-9.4 reworded as outcomes with the exploit named in their instrument; L-10.3 (S-5) added; L-9.6 (S-6), L-12.6 (S-7), L-5.5 (S-9), L-13.8 (S-10) extended or added; run-log overrides (S-9) in the gate |
...
| R2-5 | **Motion without a place, and "heavier"** — no place, no motion; "heavier" is relative to the place's own class, which moves between frames | A F2, N-2, H D-14 | Important | **RULED — D-42**: motion described with or without a named place (relative to the view where there is none); "heavier" is one table threshold for the whole loop; supersedes D-29 in part |
| R2-6 | **The description rows cannot be built as written** — v0.1.0 has no "nearby" (the coordinator's error in D-29's write-up); area answers carry no label and merge every feature of an overlay; L-13.5's "independent of any place" has no surface; valid time is per overlay; severity exists only as a colour role | A F4, F5 | Important | **RULED — D-43**: "nearby" is new and host-settable; per-alert answers with name and severity; a place-independent list of alerts shown; severity and valid time as data. The D-29 write-up's error corrected in place |
| R2-7 | **Severe weather was never examined** — MRMS ≤ 48 dBZ, one warning over moderate rain, no overlapping warnings over heavy radar; the heavy-end evidence waits on live severe weather (the server keeps about two hours); L-2.3's test has no oracle | B N-1 | Critical | **RULED — D-44**: an IEM archive specimen of a past outbreak (OW-11); RK-11; a triggered MRMS capture (OW-12); L-2.3's stated fallback if no severe day comes before SHIP |
| R2-8 | **Nothing requires the warning to stay visible inside the blend** — at 20 % the tint is faint; the checker guards only against wash-out | B N-4 | Important | **RULED — D-45**: a visibility floor of 5 inside against outside (L-11.5); outline, label and dash carry the warning where both floors cannot hold |
| R2-9 | **OW-8 (gofmt) was self-issued** — D-23 requires a ruling for every change | H-7 | Important | **RULED — D-46**: the gate gained a formatting leg, test-first, mutation-proven; OW-8 closed; recorded as a process error |
| R2-10 | **Intensity in words** (deferred by D-29) against WCAG 1.1.1 — let M1's non-visual grader decide | A F12 | Important | **RULED — D-47**: intensity in words stays deferred; the contract tells hosts to word it from `Legend()`, and legend entries say when a table is approximate |
| R2-11 | **L-5.6's retraction against watchpost's fallback** — the ship-without-radar path ships watchpost on the version L-5.6 retracts, and also loses HR-3 and HR-7 | B N-5 | Important | **RULED — D-48**: no retraction of v0.1.0 (D-37 item 5 reversed); RK-4 states the fallback's full cost. **Standing calibration: match process depth to the real blast radius** |
| R2-12 | **Record corrections** — rows that contradict each other (L-11.2 vs L-11.4, L-11.3 missing D-27's per-class tint, L-8.8 under the fallback; L-13.7 must except L-1.12); M1's non-visual arm missing from the metrics; L-8.3 omits the hatch; the D-17 specimen must be drawn over radar; the flash ceiling's scope; the state read can say "playing" while the clock is frozen; the 16-colour arm of L-8.8 is trivial; wording rules as data shape; name, span and off-precedence in the state read; README says "its version" but the code sends `0.1.0-dev`; the brief's stale lines; the register lacks evidence and status columns and misrates RK-3, RK-10, RK-5, RK-4; the per-image cap; the additive-only check on L-7.3, L-7.4, L-9.3, L-11.1; units in L-12.2; caveats in L-2.1; palette order is file-name order; the programs cite work-branch hashes that die at release (H-1); OW-7 dead; OW-2's two due dates; required reading's "only" follow-ups list; the diagram misses `tools/atlas`; L-9.2's Purge wording; on-screen section misses default-off, frame time and the light-ground fallback; A17 uncited (covered by L-13.4) | H, B, A, S | Important/Minor | **FIXED — D-40**: L-11.2/L-11.3/L-8.8 made conditional on the blend being used, and the 16-colour arm removed; L-13.7 excepts L-1.12 and v0.1.0's answers; the metrics moved into `requirements.md` with M1's non-visual arm; L-8.3 names the hatch and what carries severity inside rain; OW-2 drawn over radar with the bottom row; L-1.10g one ceiling per map; L-1.10d "playing" only while advancing, span, off-precedence, name is the host's; L-1.12 and L-13.6 as data shape; README's version wording; the brief's stale lines; the register gains Evidence and Status columns, RK-3 and RK-10 re-rated H, RK-5's residual stated (OW-10); L-12.1 per-image cap; NFR-1's additive judgement; L-12.2 in MiB; L-2.1 caveats; palette read order; the programs anchored to the `v0.1.0` tag; OW-7 deleted, OW-9 added; two owed lists named; the diagram's seventh module; L-9.2 Purge wording; the on-screen section. **A17** (ReduceMotion's overstated comment) is covered by L-13.4 and was never cited in round 1's table: cited here |
