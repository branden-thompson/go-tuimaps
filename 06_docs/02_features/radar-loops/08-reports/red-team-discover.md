...
| R2-4 | **Which frame drives the `stale` word and the description** — the shown frame makes `stale` toggle every cycle and breaks L-1.10e; the newest frame contradicts L-1.3 as worded | A F1 | Critical | pending — HUM LEAD fork |
