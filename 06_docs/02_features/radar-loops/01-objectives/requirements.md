...
| L-7.3 | **The library bounds the bytes and the time of every host fetch itself, whatever the host fetcher does.** The fetcher's shape lets the library limit what it reads and return on a deadline without waiting for the host function. `fetch.Checked` as it stands — a length check after the whole body has arrived, and no deadline — does not meet this. The contract states what a host fetcher takes on (TLS, redirects, private addresses). | D-30 (F3), D-40 (S-2), W1-B | NO INSTRUMENT YET; a test with a host fetcher that streams without end and one that ignores its context |
...
| L-9.4 | **Age runs from when the tile was fetched**, recorded apart from recency and never refreshed by a read; maximum age is enforced at `CacheRoot` and in every job; a future-dated fetch time counts as expired. | D-30 (F5), D-40 (S-3) | NO INSTRUMENT YET; a test that reads a tile hourly past its maximum age |
...
| L-13.5 | **A list of the alerts shown, needing no place**: each alert's name, severity word and valid time or stale mark. | D-29 (A11), D-43 | NO INSTRUMENT YET |
...
| # | Requirement | Source |
|---|---|---|
| NFR-1 | Additive only: a breaking change to the v0.1.0 contract is a HUM LEAD ruling. **Rows that change behaviour a v0.1.0 host can observe**, each judged not breaking: L-7.3 and L-7.4 (a host fetcher is bounded and takes effect at once — narrower failure, same call), L-9.3 (Purge empties more — the same promise, kept), L-11.1 (frames look different — pixels, ruled at D-14). Round 3 checks the judgement. | C-7, D-40 (B N-10) |
