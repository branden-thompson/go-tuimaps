...
| L2.6 | The budget's surface (L-12.1): an option and a setter; a hand-in over it is refused, saying by how much | `map.go`, `internal/overlay/store.go` | `WithImageBudget(bytes int64) Option`; `(*Map).SetImageBudget(int64) error`; default 6 MiB (D-68, raised from D-61's 3 MiB) | Over the budget: refused, and the error says by how much |
