...
|---|---|---|---|---|
| F-3 | **An image's answers ignore its projection.** `internal/describe`'s `Image` reads every image as plate carrée (`centre`, `pixel`), though an image may be handed in as web Mercator; a web-Mercator image's class at a place, its "heavier" direction and v0.2.0's motion sightings are then off in latitude, the more so the farther from the equator. The renderer honours the projection; only the answers do not. | Found while building L5.5, outside its scope: every image in the library's own tests and examples is plate carrée, and v0.1.0 shipped this way | Before SHIP of v0.2.0, since watchpost's radar source may be web Mercator; or any report of a wrong answer | BUILD, L5.5 |
